...
interface AblativeWebSocketEventMap extends WebSocketEventMap {
  down: CloseEvent;
  reopen: Event;
}

type WebSocketListener<K extends keyof AblativeWebSocketEventMap> = (
  this: WebSocket,
  event: AblativeWebSocketEventMap[K],
) => any;

type WebSocketListeners = {
  [K in keyof AblativeWebSocketEventMap]?: Array<WebSocketListener<K>>;
} & {
...
export default class AblativeWebSocket implements WebSocket {
  ...
  public readonly CONNECTING = AblativeWebSocket.CONNECTING;
  public readonly OPEN = AblativeWebSocket.OPEN;
  public readonly CLOSING = AblativeWebSocket.CLOSING;
  public readonly CLOSED = AblativeWebSocket.CLOSED;
  ...
  public get readyState(): number {
    return this.isClosed ? AblativeWebSocket.CLOSED : AblativeWebSocket.OPEN;
  }
  ...
  public addEventListener<K extends keyof AblativeWebSocketEventMap>(
    type: K,
    listener: (this: WebSocket, event: AblativeWebSocketEventMap[K]) => void,
  ): void;
  ...
  public removeEventListener<K extends keyof AblativeWebSocketEventMap>(
    type: K,
    listener: (this: WebSocket, event: AblativeWebSocketEventMap[K]) => void,
  ): void;
...
function applyDefaultOptions(options: Options): Required<Options> {
  const result: any = {};
  Object.keys(AblativeWebSocket.DEFAULT_OPTIONS).forEach((key) => {
    const value = (options as any)[key];
    result[key] =
      value === undefined
        ? (AblativeWebSocket.DEFAULT_OPTIONS as any)[key]
        : value;
  });
  return result;
}
